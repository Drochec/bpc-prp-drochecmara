...
public:
    // Constructor takes a shared_ptr to an existing node instead of creating one.
    SinClass(const rclcpp::Node::SharedPtr &node, const std::string &topic, double freq)
        : node_(node), start_time_(node_->now()) {
        ...
        // Initialize the subscriber
        subscriber_ = node_->create_subscription<std_msgs::msg::Float32>(
            topic, 1, std::bind(&SinClass::subscriber_callback, this, std::placeholders::_1));

        // Create a timer
        timer_ = node_->create_wall_timer(
            std::chrono::milliseconds(static_cast<int>(1000.0 / freq)),
            std::bind(&SinClass::timer_callback, this));
...
private:
    void timer_callback() {
        ...
        double time_sec = (node_->now() - start_time_).seconds();

        float value = std::sin(2.0 * M_PI * 1.0 * time_sec);  // 1 Hz sine wave
